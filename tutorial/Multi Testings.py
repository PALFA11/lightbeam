...
def thread_safe_save_results(folder, di, dz, initial_conditions, input_light_real, input_light_imag, powers, abs_u0_final_positions, outputs_real, outputs_imag, runtimes):
    with save_lock:  # Step 3: Acquire the lock before calling save_results
        save_results(folder, di, dz, initial_conditions, input_light_real, input_light_imag, powers, abs_u0_final_positions, outputs_real, outputs_imag, runtimes)
...
def save_results(folder, di, dz, initial_conditions, input_light_real, input_light_imag, powers, abs_u0_final_positions, outputs_real, outputs_imag, runtimes):
    ...
    results = {
        ...
        "dz": dz,
        "initial_conditions": initial_conditions,
        "input_light_real": np.asarray(input_light_real).tolist(),  # Convert to list if it's a numpy array
        "input_light_imag": np.asarray(input_light_imag).tolist(),  # Convert to list if it's a numpy array
        "powers": np.asarray(powers).tolist(),  # Convert to list if it's a numpy array
        "abs_u0_final_positions": np.asarray(abs_u0_final_positions).tolist(),  # Convert to list if it's a numpy array
        "outputs_real": np.asarray(outputs_real).tolist(),  # Convert to list if it's a numpy array
        "outputs_imag": np.asarray(outputs_imag).tolist(),  # Convert to list if it's a numpy array
        "runtimes": runtimes
    }
    ...
    try:
        ...
        print(f"Error saving results for di={di}, dz={dz}: {e}")
...
def run_simulation(di, dz, results_folder):
    ...
    thread_safe_save_results(results_folder, di, dz, initial_conditions, input_light_real, input_light_imag, powers, abs_u0_final_positions, outputs_real, outputs_imag, runtime)
...
def main():
    ...
    di_values = [0.9,0.95,0.85]
    dz_values = [100]

    results_folder = "uniform_modal_correction"
